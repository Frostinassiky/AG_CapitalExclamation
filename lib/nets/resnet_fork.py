...
class resnetv1(Network):
  ...
  def proposal_fork(self, net_conv4, part):
    ...
    proposals_y1 = tf.stack(proposals_y1)*height+part*0
    proposals_x2 = tf.stack(proposals_x2)*width
    proposals_y2 = tf.stack(proposals_y2)*height+part*0
  ...
  def build_network(self, sess, is_training=True):
    ...
    with tf.variable_scope(self._resnet_scope, self._resnet_scope):
      ...
      if cfg.POOLING_MODE == 'crop':
        ...
        raise NotImplementedError
    # HGM
    roish, roi_scoresh = proposal_fork( net_conv4, 0)
    poolh = self._crop_pool_layer(net_conv4, roish, "pool5")
    roisg, roi_scoresg = proposal_fork( net_conv4, 0)
    poolg = self._crop_pool_layer(net_conv4, roisg, "pool5")
    roism, roi_scoresm = proposal_fork( net_conv4, 0)
    poolm = self._crop_pool_layer(net_conv4, roism, "pool5")
